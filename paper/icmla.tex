...
\For{$Appliance\: n \in s$}
	...
	\Else{
		\If {$Step\_Changes(\theta^n,15,t).Times \subset Step\_Changes(\theta^{M_1},15,t).Times \linebreak \&\: 
		Step\_Changes(\theta^n,15,t).Magnitude\/ \linebreak
	...
	\textbf{Calibration} \;
	$\theta^n \gets \theta^n * Step\_Changes(\theta^n,15,t).Magnitude  \linebreak
