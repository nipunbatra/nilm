...
\noindent \textbf{Load division}: Across many countries, electrical distributions are planned such that different loads are connected to different mains (e.g. split-phase in the USA and 3-phase supply in India). Since NIALM deployment typically involves monitoring different electrical mains separately, we leverage the load division to perform efficient disaggregation. Considering $p$ mains in a home, we first perform automated load assignment (for a total of $N$ loads in the home) to individual main (resulting in $N_i$ loads being assigned to the $i^{th}$ main).% whereby $Mapping[n]$ indicates the mapping of an appliance to a main.
...
\begin{figure} 
    \subfloat[\scriptsize After applying CO based NIALM - State 2 is predicted often as State 1]{
    \includegraphics[scale=0.35]{./figures/ref_confusion_before.png}}
    \hspace{10pt}
    \subfloat[\scriptsize After applying \indicns-CO based NIALM - Significant improvement in State 2 and 3 prediction]{
