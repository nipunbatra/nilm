...
\vspace{-1mm}
\noindent Non-intrusive load monitoring was first studied by Hart~~\cite{hart} in the early 1990s by examining signatures in aggregated load to indicate activities of appliances. With proliferation of smart meters, the problem has gained traction in the recent times~\cite{survey1,survey3}. NILM systems can be broadly divided into two categories based on whether supervised or unsupervised disaggregation methods are used.
%\begin{itemize}
...
\noindent \textbf{Load division}: Across many countries, electrical distributions are planned such that different loads are connected to different mains (e.g. split-phase in the USA and 3-phase supply in India). Since NILM deployment typically involves monitoring different electrical mains separately, we leverage the load division to perform efficient disaggregation. Considering $p$ mains in a home, we first perform automated load assignment (for a total of $N$ loads in the home) to individual main (resulting in $N_i$ loads being assigned to the $i^{th}$ main).% whereby $Mapping[n]$ indicates the mapping of an appliance to a main.
~Such a load division across different mains results in exponential reduction in state space for disaggregation each main separately (the state space size for $i^{th}$ main is given by $K^{N_i}$). %As a practical example, if a home has two mains and 20 loads with 10 loads across each of the two mains, state space size before load division is given by $2^{20}$ and after load division is $2^{10}$.
...
\begin{figure} 
    \subfloat[\scriptsize Assignment of refrigerator to main 2  (Refrigerator power $>$ main 1 power)]{
...
%    \subfloat[\scriptsize Fluctuation in voltage]{
%        \label{fig:voltage}
...
%LG GL 241/2009 . Specifications of the exact model are not available, however specifications for a similar refrigerator can be found here: 
...
%\redcolor{Give rated power of refrigerator as well}. 
...
%To illustrate this we measured the voltage across a 24 hour period for many days in a home in India using EM6400 smart meter\footnote{\url{http://goo.gl/Oi98q}}. \figref{fig:voltage} shows that the voltage fluctuates between 180 and 235 Volts in a 24 hour period.
...
% For the overall dataset, it can be seen that MNE reduces from 187\% to 39\% and RE reduces from 478 W to 168 W after applying \indicns-CO.
...
\begin{figure} 
    ...
    \subfloat[\scriptsize After Calibration - Unaccounted power reduces to less than 10\% of total power]{
        ...
        \vspace{-8pt}
    \caption{Main 2 break down by load}
    \vspace{-8pt}
    \label{fig:breakdown}
    \vspace{-8pt}
\end{figure}
...
\begin{figure} 
    \subfloat[\scriptsize After applying CO based NILM - State 2 is predicted often as State 1]{
    \includegraphics[scale=0.35]{./figures/ref_confusion_before.png}}
    \hspace{10pt}
    \subfloat[\scriptsize After applying \indicns-CO based NILM - Significant improvement in State 2 and 3 prediction]{
        \includegraphics[scale=0.35]{./figures/ref_confusion_after.png}}
        \vspace{-8pt}
...
%                          ate     & events&&&\\
%                      \hline
%                      1&5070&\cellcolor{gray!25}4740&288&41\\
%                      \hline
%                      2&4810&1775&\cellcolor{gray!25}2860&176\\
%                      \hline
%                      3&200&112&63&\cellcolor{gray!25}25\\
%                      \hline
%                 \end{tabular}
%             \end{minipage}
%             \begin{minipage}{.45\linewidth}
...
%                                           ate     & events&&&\\
%                                       \hline
%                                       1&5070&\cellcolor{gray!25}4541&430&98\\
%                                       \hline
%                                       2&4810&221&\cellcolor{gray!25}4434&156\\
%                                       \hline
%                                       3&200&5&44&\cellcolor{gray!25}151\\
%                                       \hline
%                                  \end{tabular}
%                              \end{minipage}  
%\end{table}
...
\begin{table}[ht!]
\centering
\caption{MNE and RE for CO based NILM with and without \indicns. Results for \indicns-CO are highlighted in grey}
\vspace{-8pt}
\label{tab:results}
