...
\KwIn{Raw appliance, Mains 1 and 2 power data, Domain expertise about appliances	}
\KwOut{Disaggregated mains power data by appliance}
...
\For {$load$ in sorted list of background loads}
	{
	\If{$load > $ either Mains during $low\_activity$} {
	Assign $load$ to other Mains\;	
	}
	\Else{
	Find duty cycle of $load$ and compare with both Mains during $low\_activity$\;  
	Assign $load$ to the Mains having power step up and down at same time as that of $load$\;	
	}
	Subtract $load$ from assigned Mains
	}
Sort foreground loads in decreasing order of peak power\;
\For {$load$ in sorted list of foreground loads}
	{
%	Find $load\_used\_alone$ times when $load$ is used alone amongst unassigned loads and $load >$ 100 W\;
	\If{$load > $ either Mains during entire timeseries} {
	Assign $load$ to other Mains\;	
	}
	\Else{
	Find time windows when $load >$ 100 W and find step changes occurring and their $times$ in $load$ during these time windows\;
	Assign $load$ to the Mains where similar step changes are found in same $times$ during the time windows\;
	}
	Subtract $load$ from assigned Mains
	}
\For {$load$ in sorted list of loads}
{
...
\Return{Disaggregated mains power data per appliance}\;
\caption{INDiC}
\label{algo:change}
\end{algorithm}
