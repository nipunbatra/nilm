...
\hline
\multirow{2}{*}{$Downsample(s,filter)$} & Function to downsample a timeseries $s$\\
                                        & according to specified filter: mean/median\\
...
\KwIn{$x,\theta^n,\theta^{M_1},\theta^{M_2},B,F$	}
\KwOut{Disaggregated mains power data by appliance}
\textbf{Align and Downsample}\;
\For {$n \in {1,N}$ in sorted list of background loads}{nothng}
